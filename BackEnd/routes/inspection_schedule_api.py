...
def get_inspection_schedule_by_id(current_manager=None, _id: str = ''):
    try:
        the_inspection_schedule = inspection_schedule.find_one({'_id': ObjectId(_id)})
        if the_inspection_schedule:
            return the_inspection_schedule
        else:
...
@app.route('/inspection_schedule/search', methods=['GET'])
# @manager_required('level_two')
def get_inspection_schedule(current_manager=None):
    try:
        ...
        if some_inspection_schedule:
            ...
            return pagination_inspection_schedule(path_dir=f'inspection_schedule/search',
                                                  offset=offset,
                                                  limit=limit,
